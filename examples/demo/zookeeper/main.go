...
import (
	...
	"github.com/topfreegames/pitaya/v2/examples/demo/zookeeper/services"
...
func main() {
	...
	var bs *modules.DemoBindingStorage
...
func createApp(port int, isFrontend bool, svType string, meta map[string]string, rpcServerPort int) (pitaya.Pitaya, *modules.DemoBindingStorage) {
	...
	bs := modules.NewDemoBindingStorage()
	...
	if isFrontend {
		...
		ws := acceptor.NewWSAcceptor(fmt.Sprintf("0.0.0.0:%d", port))
		builder.AddAcceptor(ws)
	}

	// service discovery
	ds, err := cluster.NewZookeeperServiceDiscovery(nil, builder.Server)
	if err != nil {
		fmt.Printf("error NewZookeeperServiceDiscovery %s\n", err.Error())
	}
	builder.ServiceDiscovery = ds
